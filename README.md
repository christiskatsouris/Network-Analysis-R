...
> Notice that a Dynamic Panel Data Model specification for the estimation problem under examination would indeed considered to be a good starting point. However, due to the presence of temporal dependence and possible network effects, the particular modeling approach would require additional refinements. Specifically, the given structure of the data, which are obtained sequentially through time (i.e., generated from the optimization step), implies that a more suitable estimation methodology can be found in the statistical literature for modeling weakly dependent functional data. Related references are the studies of   
