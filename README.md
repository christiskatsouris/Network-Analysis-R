...
$\textbf{Lemma:}$ A vector $\mathbf{x}=(x_1,...,x_n)$ is said to be an eigenvctor of an (N x N matrix) $\mathbf{A}$ with an eigenvalue $\lambda$ if for each $i$ it satisfies the equation 
$$\sum_{j=1}^N a_{ij}x_j = \lambda x_i.$$ 
The eigenvalues of a matrix $\mathbf{A}$ are roots of the characteristic equation of the matrix $| \mathbf{A} - \lambda \mathbf{I} | = 0$.   

$\textbf{Theorem:}$ (Perron theorem) If A is a positive matrix, there is a unique characteristic root of A, $\lambda(A)$, which has the greatest absolute value. This root is positive and simple, and its associated characteristic vector may be taken to be positive (see e.g., \cite{bellman1997introduction}). 
